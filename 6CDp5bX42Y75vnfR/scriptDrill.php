...
$response = file_get_contents('https://www.googleapis.com/youtube/v3/search?key=AIzaSyBsHl2ULTmyVwbjuFuFzD4EVnu4UfXSn-w&maxResults=50&part=snippet&q=Type%20Beat%20Drill');
...
while ($pagecours < $pagemax){
    $json = file_get_contents('https://www.googleapis.com/youtube/v3/search?key=AIzaSyBsHl2ULTmyVwbjuFuFzD4EVnu4UfXSn-w&maxResults=50&part=snippet&q=Type%20Beat%20Drill&pageToken='.$pageToken);
...
$fichier = fopen('drill.json', 'w+b');

fwrite($fichier, $stringjson);
fclose($fichier);

$drilljson = file_get_contents('drill.json');
...
for ($p=0; $p< $sizevideoid; $p++){
    ...
    $req = "INSERT INTO `api`(`Titreapi`, `lienapi`, `id_apigenre`) VALUES ('".$title."','".$videoid."',1);";
...
echo "<script type='text/javascript'>document.location.replace('/Admin_kam_maj_onglets/mise-a-jour/script-Drill');</script>";
}else{
        header('Location: /');
    }
}else{

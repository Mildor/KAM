...
$response = file_get_contents('https://www.googleapis.com/youtube/v3/search?key=AIzaSyAA-rQHAQUiGzGe18T2c_3hrmMJk_2w_cE&maxResults=50&part=snippet&q=Type%20Beat%20Piano');
...
while ($pagecours < $pagemax){
    $json = file_get_contents('https://www.googleapis.com/youtube/v3/search?key=AIzaSyAA-rQHAQUiGzGe18T2c_3hrmMJk_2w_cE&maxResults=50&part=snippet&q=Type%20Beat%20Piano&pageToken='.$pageToken);
...
$fichier = fopen('piano.json', 'w+b');

fwrite($fichier, $stringjson);
fclose($fichier);

$drilljson = file_get_contents('piano.json');
...
for ($p=0; $p< $sizevideoid; $p++){
    ...
    $req = "INSERT INTO `api`(`Titreapi`, `lienapi`, `id_apigenre`) VALUES ('".$title."','".$videoid."',4);";
...
echo "<script type='text/javascript'>document.location.replace('/Admin_kam_maj_onglets/mise-a-jour/script-Piano');</script>";
}else{
        header('Location: /');
    }
}else{

...
$response = file_get_contents('https://www.googleapis.com/youtube/v3/search?key=AIzaSyAsKYKwlegA7YFz23yTY3-600r8SnrxOHQ&maxResults=50&part=snippet&q=Type%20Beat%20Trap');
...
while ($pagecours < $pagemax){
    $json = file_get_contents('https://www.googleapis.com/youtube/v3/search?key=AIzaSyAsKYKwlegA7YFz23yTY3-600r8SnrxOHQ&maxResults=50&part=snippet&q=Type%20Beat%20Trap&pageToken='.$pageToken);
...
$fichier = fopen('trap.json', 'w+b');

fwrite($fichier, $stringjson);
fclose($fichier);

$drilljson = file_get_contents('trap.json');
...
for ($p=0; $p< $sizevideoid; $p++){
    ...
    $req = "INSERT INTO `api`(`Titreapi`, `lienapi`, `id_apigenre`) VALUES ('".$title."','".$videoid."',2);";
...
echo "<script type='text/javascript'>document.location.replace('/Admin_kam_maj_onglets/mise-a-jour/script-Trap');</script>";
}else{
        header('Location: /');
    }
}else{

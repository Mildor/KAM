...
$response = file_get_contents('https://www.googleapis.com/youtube/v3/search?key=AIzaSyDa8ftRI2HTVuR9XoM4Gbe8nGZhVboiZlE&maxResults=50&part=snippet&q=Type%20Beat%20CloudRap');
...
while ($pagecours < $pagemax){
    $json = file_get_contents('https://www.googleapis.com/youtube/v3/search?key=AIzaSyDa8ftRI2HTVuR9XoM4Gbe8nGZhVboiZlE&maxResults=50&part=snippet&q=Type%20Beat%20CloudRap&pageToken='.$pageToken);
...
$fichier = fopen('cloudrap.json', 'w+b');

fwrite($fichier, $stringjson);
fclose($fichier);

$drilljson = file_get_contents('cloudrap.json');
...
for ($p=0; $p< $sizevideoid; $p++){
    ...
    $req = "INSERT INTO `api`(`Titreapi`, `lienapi`, `id_apigenre`) VALUES ('".$title."','".$videoid."',7);";
...
echo "<script type='text/javascript'>document.location.replace('/Admin_kam_maj_onglets/mise-a-jour/script-CloudRap');</script>";
}else{
        header('Location: /');
    }
}else{

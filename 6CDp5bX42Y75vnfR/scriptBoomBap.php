...
$response = file_get_contents('https://www.googleapis.com/youtube/v3/search?key=AIzaSyDvxCsSzv6oYDyqWCiJjYUI-GV09KzI2KM&maxResults=50&part=snippet&q=Type%20Beat%20BoomBap');
...
while ($pagecours < $pagemax){
    $json = file_get_contents('https://www.googleapis.com/youtube/v3/search?key=AIzaSyDvxCsSzv6oYDyqWCiJjYUI-GV09KzI2KM&maxResults=50&part=snippet&q=Type%20Beat%20BoomBap&pageToken='.$pageToken);
...
$fichier = fopen('BoomBap.json', 'w+b');

fwrite($fichier, $stringjson);
fclose($fichier);

$drilljson = file_get_contents('boomBap.json');
...
for ($p=0; $p< $sizevideoid; $p++){
    ...
    $req = "INSERT INTO `api`(`Titreapi`, `lienapi`, `id_apigenre`) VALUES ('".$title."','".$videoid."',3);";
...
echo "<script type='text/javascript'>document.location.replace('/Admin_kam_maj_onglets/mise-a-jour/script-BoomBap');</script>";
}else{
        header('Location: /');
    }
}else{

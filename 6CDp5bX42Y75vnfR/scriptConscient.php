...
$response = file_get_contents('https://www.googleapis.com/youtube/v3/search?key=AIzaSyBZG3JFeOB9gq-2f8GnrzukzDv9IxK7Yro&maxResults=50&part=snippet&q=Type%20Beat%20Conscient');
...
while ($pagecours < $pagemax){
    $json = file_get_contents('https://www.googleapis.com/youtube/v3/search?key=AIzaSyBZG3JFeOB9gq-2f8GnrzukzDv9IxK7Yro&maxResults=50&part=snippet&q=Type%20Beat%20Conscient&pageToken='.$pageToken);
...
$fichier = fopen('conscient.json', 'w+b');

fwrite($fichier, $stringjson);
fclose($fichier);

$drilljson = file_get_contents('conscient.json');
...
for ($p=0; $p< $sizevideoid; $p++){
    ...
    $req = "INSERT INTO `api`(`Titreapi`, `lienapi`, `id_apigenre`) VALUES ('".$title."','".$videoid."',6);";
...
echo "<script type='text/javascript'>document.location.replace('/Admin_kam_maj_onglets/mise-a-jour/script-Conscient');</script>";
}else{
        header('Location: /');
    }
}else{
